...
export const RESOURCES: Record<string, Resource> = {
  ...
  recognitions: {
    table: "recognitions",
    label: "About · Recognitions / Awards",
    singular: "Recognition",
    titleField: "text",
    hideFromNav: true,
    fields: [
      { name: "text", label: "Citation (e.g. \"Best Wedding Planner 2026 — WeddingWire India\")", type: "text" },
      { name: "sort_order", label: "Order", type: "number" },
    ],
  },
  destinations: {
    table: "destinations",
    label: "About · Destinations",
    singular: "Destination",
    titleField: "name",
    subtitleField: "region",
    hideFromNav: true,
    fields: [
      { name: "name", label: "City / place name", type: "text" },
      { name: "region", label: "Region", type: "select", options: ["Domestic", "International"] },
      { name: "sort_order", label: "Order", type: "number" },
    ],
  },
  process_phases: {
  ...
  site_settings: {
    ...
    fields: [
      ...
      { name: "collage_images", label: "Home collage photos (grid — upload each tile)", type: "imagelist", help: "Add as many photos as you like, in order — the grid grows to fit them. The \"Celebrating Love\" text tile stays fixed near the top of the grid." },
      { name: "collage_image", label: "OR: single ready-made collage banner", type: "image", help: "Only used if the grid above is empty — one pre-made image spanning the whole section instead of a tiled grid." },
